...
/* 		$('#lab').show()
		...
		$('.infoIcon').off('click').click(function(){	
			lastplayedAudio	= AudioNameget;
			globalAudioPause();
			$('.helpinner .shadowTop').hide();
			$('.helpinner .shadowBottom').css({bottom:'104px'});
		...
		$("#rightlabel_1").trigger("click");
				...
				$('iframe').contents().find("#axe_aud").trigger('pause');
				audioPlay();
	}
 var closeHelpWin = function(e){
	
	$('.helpwindow').fadeOut(300);
	AudioNameget = lastplayedAudio;	
	globalAudioPause();
...
function rbuttoneve(event)
	{	
		...
		ctTarget.css({'color':'#FFCC00','box-shadow':'0 -6px #FFCB05'});
		$('.workArea').hide();
		$('#'+cpage).show();		
		$('[name = submitconfirmation]').removeClass('subbtnActive').addClass('subbtn');
		lightString = '';
		activeCount = 0;				
		exploreAndActivity(cpage);
	...
	{ 
		tocanvas($(".screenArea"), {
			onrendered: function (canvas) {
				saveAs(canvas.toDataURL(), 'Fossil Dig.png');
			}
		});
	}
